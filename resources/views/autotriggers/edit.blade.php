        ...
        <div class="col-lg-8 offset-lg-2">
            <div class="card">
                <div class="card-header">
                    @if($autotrigger->from_type == 'Telegram')
                        {{ __('Edit Telegram Autotrigger') }}
                    @else
                        {{ __('Edit Email Autotrigger') }}
                    @endif
                </div>
                <div class="card-body">
                    <form action="{{ route('autotrigger.update', $autotrigger->id) }}" method="POST" class="form-horizontal">
                        @csrf
                        @method('PUT')
                        <x-sendportal.text-field name="name" :label="__('Name')" :value="$autotrigger->name ?? old('name')" />

                         <input type="hidden" name="from_type" value="{{ $autotrigger->from_type }}" />
                        ...
                        <div class="form-group row">
                            <div class="offset-sm-3 col-sm-9">
                                <a href="{{ route('autotrigger.index',['type'=>$autotrigger->from_type]) }}" class="btn btn-light">{{ __('Cancel') }}</a>
